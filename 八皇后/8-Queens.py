...
class Board:
    def __init__(self, n):
        self.totalqueens = n #h皇后数量
        self.totalPositon = n * n #棋盘大小
    ...
    def add_or_remove_constraints(self, move, add):
        ...
        row = move // self.totalqueens #向下取整除法
...
def solveNQueens(b, j):
    ...
    """
    possiblemoves = b.get_possible_moves() #得到所有可以放的位置
    col = 0
    while col < 8:
        move = b.get_pos(j, col)
        col = col + 1
        if move in possiblemoves:
            b.make_move(move)
            if j < 7:
                result = solveNQueens(b, j + 1)
            else:
                return 0
            if result != None:
                return 0
...
if __name__ == '__main__':
    ...
    b.make_move(r)     #走完了第一步
    a = 0

    if solveNQueens(b, j=1) == 0:
        print('求解成功！')
        b.print_board()
    else:
        print('无解！')
